...
On Error GoTo LOKAL_ERROR
    ...
    Select Case Index
        Case 0      'Speichern
            ...
            insertStaffelpreis CLng(gsARTNR), Val(Text1(0).Text), CSng(Text1(1).Text), CLng(Label1(3).Caption)
            LeseStaffelpreisinList CLng(gsARTNR), List1, CLng(gsLinr)

            Unload frmWKL161
        Case 1     'Löschen
            deleteStaffelpreis CLng(gsARTNR)
            LeseStaffelpreisinList CLng(gsARTNR), List1, CLng(gsLinr)
...
LOKAL_ERROR:
    ...
    Fehlermeldung1
End Sub
Private Sub deleteStaffelpreis(lartnr As Long)
    On Error GoTo LOKAL_ERROR
    
    Dim sSQL As String

    sSQL = "Delete from Staffelpr where ARTNR = " & lartnr
...
LOKAL_ERROR:
    ...
    Fehlermeldung1
End Sub
Private Sub insertStaffelpreis(lartnr As Long, lMenge As Long, siPreiswert As Single, lLinr As Long)
    On Error GoTo LOKAL_ERROR
    
    Dim sSQL As String

    sSQL = "Delete from Staffelpr where ARTNR = " & lartnr
    sSQL = sSQL & " and Menge = " & lMenge
    sSQL = sSQL & " and LINR = " & lLinr
    gdBase.Execute sSQL, dbFailOnError
    
    If siPreiswert > 0 Then
    
        sSQL = "Insert into Staffelpr (Artnr,Menge,LEKPR,AENDER,LINR) values  "
        sSQL = sSQL & " ( " & lartnr
        sSQL = sSQL & " , " & lMenge
        sSQL = sSQL & " , '" & siPreiswert & "'"
...
LOKAL_ERROR:
    ...
    Fehler.gsNumber = err.Number
    Fehler.gsFormular = Me.name
    Fehler.gsFunktion = "insertStaffelpreis"
    Fehler.gsFehlertext = "Im Programmteil Staffelpreise ist ein Fehler aufgetreten."
