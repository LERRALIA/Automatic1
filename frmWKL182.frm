...
Begin VB.Form frmWKL182 
   Caption         =   "Etiketten wiederherstellen"
   ClientHeight    =   8595
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11880
...
LOKAL_ERROR:
    ...
    Fehler.gsFehlertext = "Im Programmteil Etiketten wiederherstellen ist ein Fehler aufgetreten."
...
On Error GoTo LOKAL_ERROR
    ...
    If bFound Then
        iRet = MsgBox("Wollen Sie nur die markierten Etiketten wiederherstellen?", vbYesNoCancel + vbQuestion, "Winkiss Frage:")
        ...
        ElseIf iRet = vbYes Then
            ...
            For lcount = 0 To List3.ListCount - 1
                If List3.Selected(lcount) = True Then
                    ...
                    cArtNr = Trim$(cArtNr)
                    cFilnr = Trim$(Right$(Trim(cLBSatz), 2))
                    
                    ZurückSicherInEtidru cArtNr, cFilnr
                End If
            Next lcount
        Else
            iRet = MsgBox("Wollen Sie wirklich alle Etiketten wiederherstellen?", vbYesNoCancel + vbQuestion, "Winkiss Frage:")
            If iRet = vbCancel Then
                Screen.MousePointer = 0
                Exit Sub
            ElseIf iRet = vbYes Then
                ZurückSicherInEtidruALL
            End If
        End If
    Else
        iRet = MsgBox("Wollen Sie wirklich alle Etiketten wiederherstellen?", vbYesNoCancel + vbQuestion, "Winkiss Frage:")
...
Private Sub Form_Load()
    ...
    sSQL = "Delete from ETISIC where DELDATE < datevalue(now) - 7"
    gdBase.Execute sSQL, dbFailOnError
    
    FuelleListeEtikettenWKL170
...
LOKAL_ERROR:
    ...
    Fehlermeldung1
End Sub
Private Sub FuelleListeEtikettenWKL170()
    On Error GoTo LOKAL_ERROR
    ...
    List1.AddItem "Datum  Zeit  ArtNr. Artikelbezeichnung                     VK-Preis Anz.Etiketten Fil"
    
    cSQL = "Select * from ETISIC "
...
LOKAL_ERROR:
    ...
    Fehler.gsNumber = err.Number
    Fehler.gsFormular = Me.name
    Fehler.gsFunktion = "FuelleListeEtikettenWKL170"
