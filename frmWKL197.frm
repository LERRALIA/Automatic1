...
Begin VB.Form frmWKL197 
   Caption         =   "SQL Server Upgrade"
   ClientHeight    =   8595
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11880
   Icon            =   "frmWKL197.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   8595
   ...
   End
   Begin VB.Label Label1 
      Caption         =   "Adresse/Rechnername/IP"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   End
   Begin VB.Label Label1 
      Caption         =   "SQL Server Verbindungsdaten"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   Begin VB.Label lbl6 
      BackColor       =   &H00C0C0FF&
      Caption         =   "SQL SERVER lokal oder im Netzwerk"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   12
         Charset         =   0
   ...
   Begin VB.Label lbl6 
      BackColor       =   &H00C0C0FF&
      Caption         =   "Was benötigen Sie für den Datenbankwechsel ?"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   14.25
         Charset         =   0
...
LOKAL_ERROR:
    ...
    Fehler.gsFehlertext = "Im Programmteil SQL-Server Upgrade ist ein Fehler aufgetreten."
    
    Fehlermeldung1
End Sub
Private Sub speicher_dieSS_Adresse()
On Error GoTo LOKAL_ERROR
...
LOKAL_ERROR:
    ...
    Fehler.gsNumber = err.Number
    Fehler.gsFormular = Me.name
    Fehler.gsFunktion = "speicher_dieSS_Adresse"
    Fehler.gsFehlertext = "Im Programmteil SQL-Server Upgrade ist ein Fehler aufgetreten."
    
    Fehlermeldung1
End Sub
Public Sub ImportSQL_Server(lblx As Label)
On Error GoTo LOKAL_ERROR

    speicher_dieSS_Adresse

    Dim dbSQLS As Database

    gsKL_ADRESSE = "NOT_ASUS\SQLEXPRESS"
    gsKL_DATENBANKNAME = "stada"
    gsKL_BENUTZER = "sa"
    gsKL_PASSWORT = "sigverif"
'
'    gsKL_ADRESSE = "80.86.85.121"
'    gsKL_DATENBANKNAME = "HEINZ"
'    gsKL_BENUTZER = "kadb17"
'    gsKL_PASSWORT = "sigverif1"
    
'    gsKL_ADRESSE = "MATTHIAS-NB"
'    gsKL_DATENBANKNAME = "Heinz"
'    gsKL_BENUTZER = "sa"
'    gsKL_PASSWORT = "kisslive"
    ...
    Set dbSQLS = OpenDatabase(gsKL_DATENBANKNAME, dbDriverNoPrompt, False, stConnect)
...
'''    dbSQLS.TableDefs.Refresh
'''
'''    lAnzTable = dbSQLS.TableDefs.Count
'''
'''    For lcount = 0 To lAnzTable - 1
'''        name = dbSQLS.TableDefs(lcount).name
'''
'''        If Left(name, 12) = "INFORMATION_" Then
'''
'''        ElseIf Left(name, 4) = "sys." Then
'''
'''        Else
'''            sSQL = "Drop Table " & name
'''            dbSQLS.Execute sSQL, dbSQLPassThrough + dbFailOnError
'''
'''            anzeige "normal", "Datenbank wird aufgeräumt: " & name, lblx
'''        End If
'''    Next lcount
'''
...
'''
'''    dbWK.TableDefs.Refresh
'''
'''    lAnzTable = dbWK.TableDefs.Count
'''
'''    For lcount = 0 To lAnzTable - 1
'''        name = Trim(dbWK.TableDefs(lcount).name)
'''
'''        anzeige "normal", "Tabellen werden importiert: " & name, lblx
'''
'''        sSQL = "Select * "
'''        sSQL = sSQL & " into " & name & " from [;DATABASE=" & slokalPfad & "KISSDATA.MDB;pwd=" & gsPasswort & "]." & name & ""
'''        dbSQLS.Execute sSQL, dbFailOnError
'''    Next lcount
...
'
'
'    sConn = "ODBC;DSN=" & sDSNName & ";uid=sa;pwd=" & sPassword & ""
'    Set gdBase = DBEngine.OpenDatabase(sDSNName, dbDriverNoPrompt, False, sConn)
    
    
    Set gdBase = OpenDatabase(gsKL_DATENBANKNAME, dbDriverNoPrompt, False, stConnect)
    
    gbSQLSERVER = True
    
    ReIndiziereArtikel_ForSS gdBase
    
   
    
    
    anzeige "normal", "Fertig!", lblx
...
'    Fehler.gsNumber = err.Number
'    Fehler.gsFormular = Me.name
'    Fehler.gsFunktion = "ImportSQL_Server"
'    Fehler.gsFehlertext = "Im Programmteil SQL SERVER Import ist ein Fehler aufgetreten."
'
'    Fehlermeldung1
...
'    sSQL = "Drop Table kassjour"
'    dbSQLS.Execute sSQL, dbSQLPassThrough + dbFailOnError
'
'
'
'    Dim cTemp_Tab As String
'    cTemp_Tab = "Kassjour"
...
''    sSQL = sSQL & ", 1 as ABOK "
'
'    sSQL = sSQL & " into kassjour from [;DATABASE=" & slokalPfad & "KISSDATA.MDB;pwd=" & gsPasswort & "]." & cTemp_Tab & ""
'    dbSQLS.Execute sSQL, dbFailOnError
